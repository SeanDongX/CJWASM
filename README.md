...
- **完整的类型系统** — Int8~Int64、UInt8~UInt64、Float32/64、Bool、Rune、String、Array、Tuple、Option、Result
...
| **函数** | 函数定义, 递归, 默认参数, 可变参数, Lambda, 函数类型 | ✅ |
| **泛型** | 泛型函数/结构体/类/枚举, 类型约束, 多重约束, where 子句, 泛型特化 | ✅ |
| **OOP** | 类, 继承(open/<:), abstract/sealed, 接口, 默认实现, 属性(prop) | ✅ |
| **控制流** | if/else, while, for-in, loop, break/continue, match, while-let | ✅ |
| **模式匹配** | 枚举解构, 结构体解构, if-let, while-let, guard(where), 嵌套解构 | ✅ |
...
| **运算符** | 算术, 比较, 逻辑, 位运算, 幂运算(**), 字符串插值, 类型转换(as) | ✅ |
| **标准库** | 数学函数(sin/cos/exp/log/...), 字符串操作(contains/indexOf/replace) | ✅ |
