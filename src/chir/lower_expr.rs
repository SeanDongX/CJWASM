...
use crate::chir::{CHIRBlock, CHIRExpr, CHIRExprKind, CHIRMatchArm, CHIRPattern};
...
impl<'a> LoweringContext<'a> {
    ...
    pub fn lower_expr(&mut self, expr: &Expr) -> Result<CHIRExpr, String> {
        ...
        let kind = match expr {
            ...
            Expr::Binary {
                ...
                // 确保两个操作数类型一致
                let target_ty = if func_name == "__pow_f64" {
                    ValType::F64
                } else {
                    ValType::I64
                };
            ...
            Expr::Binary { op, left, right } => {
                ...
                let effective_wasm_ty = if !is_comparison {
                    if left_chir.wasm_ty == ValType::F64 || right_chir.wasm_ty == ValType::F64 {
                        ValType::F64
                    } else if left_chir.wasm_ty == ValType::I64
                        || right_chir.wasm_ty == ValType::I64
                    {
                        ValType::I64
                    } else {
                        wasm_ty
                ...
                let (left_chir, right_chir) = if is_comparison {
                    let operand_ty = if left_chir.wasm_ty == ValType::I64
                        || right_chir.wasm_ty == ValType::I64
                    {
                        ValType::I64
                    } else if left_chir.wasm_ty == ValType::F64
                        || right_chir.wasm_ty == ValType::F64
                    {
                        ValType::F64
                    } else {
                        left_chir.wasm_ty
            ...
            Expr::Call { name, args, .. } => {
                ...
                match name.as_str() {
                    "println" | "print" | "eprintln" | "eprint" => {
                        ...
                        let fd = if matches!(name.as_str(), "eprint" | "eprintln") {
                            2
                        } else {
                            1
                        };
                    ...
                    "min" | "max" if args.len() == 2 => {
                        ...
                        let then_block = crate::chir::CHIRBlock {
                            stmts: vec![],
                            result: Some(Box::new(a.clone())),
                        };
                        let else_block = crate::chir::CHIRBlock {
                            stmts: vec![],
                            result: Some(Box::new(b.clone())),
                        };
                        return Ok(CHIRExpr::new(
                            CHIRExprKind::If {
                                cond: Box::new(cond),
                    ...
                    "sqrt" | "floor" | "ceil" | "trunc" | "nearest" | "abs" | "sin" | "cos"
                    | "exp" | "log"
                        if args.len() == 1
                    ...
                    "sort" if args.len() == 1 => {
                        ...
                        let arr_local = self.alloc_local_typed(
                            format!("__sort_arr_{}", self.next_local),
                            ValType::I32,
                        );
                        let n_local = self.alloc_local_typed(
                            format!("__sort_n_{}", self.next_local),
                            ValType::I64,
                        );
                        let i_local = self.alloc_local_typed(
                            format!("__sort_i_{}", self.next_local),
                            ValType::I64,
                        );
                        let j_local = self.alloc_local_typed(
                            format!("__sort_j_{}", self.next_local),
                            ValType::I64,
                        );
                        let tmp_local = self.alloc_local_typed(
                            format!("__sort_tmp_{}", self.next_local),
                            ValType::I64,
                        );
                        ...
                        stmts.push(crate::chir::CHIRStmt::Let {
                            ...
                            value: CHIRExpr::new(
                                CHIRExprKind::Cast {
                                    expr: Box::new(CHIRExpr::new(
                                        CHIRExprKind::FieldGet {
                                            object: Box::new(CHIRExpr::new(
                                                CHIRExprKind::Local(arr_local),
                            ...
                            CHIRExprKind::Binary {
                                ...
                                left: Box::new(CHIRExpr::new(
                                    CHIRExprKind::Binary {
                                        op: crate::ast::BinOp::Sub,
                                        left: Box::new(CHIRExpr::new(
                                            CHIRExprKind::Local(n_local),
                                            crate::ast::Type::Int64,
                                            ValType::I64,
                                        )),
                                        right: Box::new(CHIRExpr::new(
                                            CHIRExprKind::Local(i_local),
                                            crate::ast::Type::Int64,
                                            ValType::I64,
                                        )),
                                    },
                        ...
                        let j_cond = CHIRExpr::new(
                            CHIRExprKind::Binary {
                                op: crate::ast::BinOp::Lt,
                                left: Box::new(CHIRExpr::new(
                                    CHIRExprKind::Local(j_local),
                                    crate::ast::Type::Int64,
                                    ValType::I64,
                                )),
                        ...
                        let j_plus1_local = self.alloc_local_typed(
                            format!("__sort_j1_{}", self.next_local),
                            ValType::I64,
                        );

                        // inner body: if arr[j] > arr[j+1] { swap }; j = j + 1
                        let swap_cond = CHIRExpr::new(
                            CHIRExprKind::Binary {
                            ...
                            stmts: vec![
                                ...
                                crate::chir::CHIRStmt::Expr(CHIRExpr::new(
                                    CHIRExprKind::Store {
                                        ...
                                        value: Box::new(CHIRExpr::new(
                                            CHIRExprKind::Local(tmp_local),
                                            crate::ast::Type::Int64,
                                            ValType::I64,
                                        )),
                                        offset: 0,
                        ...
                        let inner_body = crate::chir::CHIRBlock {
                            stmts: vec![
                                ...
                                crate::chir::CHIRStmt::Expr(CHIRExpr::new(
                                    CHIRExprKind::If {
                                        cond: Box::new(swap_cond),
                                        then_block: swap_body,
                                        else_block: None,
                                    },
                                    crate::ast::Type::Unit,
                                    ValType::I32,
                                ...
                                crate::chir::CHIRStmt::Assign {
                                    ...
                                    value: CHIRExpr::new(
                                        CHIRExprKind::Local(j_plus1_local),
                                        crate::ast::Type::Int64,
                                        ValType::I64,
                                    ),
                                },
                        ...
                        stmts.push(crate::chir::CHIRStmt::While {
                            cond: outer_cond,
                            body: outer_body,
                        });
                ...
                let (func_idx_opt, needs_this) =
                    if let Some(&idx) = self.func_indices.get(mangled.as_str()) {
                        (Some(idx), false)
                ...
                let func_idx = match func_idx_opt {
                    ...
                    None => {
                        ...
                        if let Some(crate::ast::Type::Function {
                            params: fn_params,
                            ret,
                        }) = self.local_ast_types.get(name).cloned()
                        {
                            ...
                            let callee_expr = CHIRExpr::new(
                                CHIRExprKind::Local(callee_local),
                                crate::ast::Type::Int32,
                                ValType::I32,
                            );
                            let ret_wasm = ret
                ...
                let actual_arity = if needs_this {
                    args.len() + 1
                } else {
                    args.len()
                };
                ...
                // lower 位置参数
                let positional_args: Vec<CHIRExpr> = args
                    .iter()
                    .enumerate()
                    .map(|(i, a)| {
            ...
            Expr::MethodCall {
                ...
                if let Expr::Var(cls_name) = object.as_ref() {
                    ...
                    if is_class {
                        ...
                        let arity_mangled =
                            format!("{}${}", mangled, args.len() + named_args.len());
                        let func_idx = self
                            .func_indices
                            .get(&arity_mangled)
                ...
                let obj_ty = if let Expr::Var(name) = object.as_ref() {
                    self.local_ast_types.get(name).cloned().unwrap_or_else(|| {
                        self.type_ctx
                            .infer_expr(object)
                            .unwrap_or(crate::ast::Type::Int32)
                    })
                } else {
                ...
                // ── 内置类型方法处理 ──
                if let Some(result) =
                    self.try_lower_builtin_method(object, &obj_ty, method, args)?
                {
                    return Ok(result);
                }
                ...
                if (method == "init" || method.is_empty()) && self.current_class.is_some() {
                    ...
                    if let Some(parent) = self.class_extends.get(&class_name).cloned() {
                        if let Some(fields) = self.class_field_info.get(&parent) {
                            ...
                            let block = crate::chir::CHIRBlock {
                                stmts,
                                result: None,
                            };
                            return Ok(CHIRExpr::new(
            ...
            // 数组
            Expr::Array(elems) => {
                let elements: Vec<CHIRExpr> = elems
                    .iter()
            ...
            Expr::TupleIndex { object, index } => {
                ...
                let elem_ty = if let Expr::Var(name) = object.as_ref() {
                    self.local_ast_types
                        .get(name)
                        .and_then(|t| {
                            if let crate::ast::Type::Tuple(types) = t {
            ...
            Expr::StructInit { name, fields, .. } => {
                let fields_chir: Result<Vec<(String, CHIRExpr)>, String> = fields
                    .iter()
                    .map(|(fname, fexpr)| {
            ...
            Expr::ConstructorCall { name, args, .. } => {
                ...
                if name == "AtomicInt64" {
                    ...
                    let ptr_local =
                        self.alloc_local_typed(format!("__tmp_{}", self.next_local), ValType::I32);
                    let mut stmts = Vec::new();
                    stmts.push(crate::chir::CHIRStmt::Let {
                        local_idx: ptr_local,
                        value: CHIRExpr::new(
                    ...
                    stmts.push(crate::chir::CHIRStmt::Expr(CHIRExpr::new(
                        CHIRExprKind::Store {
                            ptr: Box::new(CHIRExpr::new(
                                CHIRExprKind::Local(ptr_local),
                                crate::ast::Type::Int32,
                                ValType::I32,
                            )),
                            value: Box::new(CHIRExpr::new(
                                CHIRExprKind::Integer(init_val),
                                crate::ast::Type::Int64,
                                ValType::I64,
                            )),
                            offset: 0,
                    ...
                    return Ok(CHIRExpr::new(
                        CHIRExprKind::Block(crate::chir::CHIRBlock {
                            ...
                            result: Some(Box::new(CHIRExpr::new(
                                CHIRExprKind::Local(ptr_local),
                                crate::ast::Type::Struct("AtomicInt64".to_string(), vec![]),
                                ValType::I32,
                            ))),
                        }),
                        crate::ast::Type::Struct("AtomicInt64".to_string(), vec![]),
                ...
                if name == "AtomicBool" {
                    ...
                    return Ok(CHIRExpr::new(
                        CHIRExprKind::Block(crate::chir::CHIRBlock {
                            ...
                            result: Some(Box::new(CHIRExpr::new(
                                CHIRExprKind::Local(ptr_local),
                                crate::ast::Type::Struct("AtomicBool".to_string(), vec![]),
                                ValType::I32,
                            ))),
                        }),
                        crate::ast::Type::Struct("AtomicBool".to_string(), vec![]),
                ...
                if name == "Mutex" || name == "ReentrantMutex" {
                    ...
                    return Ok(CHIRExpr::new(
                        CHIRExprKind::Block(crate::chir::CHIRBlock {
                            ...
                            result: Some(Box::new(CHIRExpr::new(
                                CHIRExprKind::Local(ptr_local),
                                crate::ast::Type::Struct(struct_name.clone(), vec![]),
                                ValType::I32,
                            ))),
                        }),
                        crate::ast::Type::Struct(struct_name, vec![]),
                        ValType::I32,
                    ));
                }
                ...
                if name == "Array" && args.len() == 2 {
                        ...
                        CHIRExprKind::Binary {
                            ...
                            right: Box::new(CHIRExpr::new(
                                CHIRExprKind::Binary {
                                    ...
                                    left: Box::new(n_get()),
                                    right: Box::new(CHIRExpr::new(
                                        CHIRExprKind::Integer(elem_size),
                                        crate::ast::Type::Int32,
                                        ValType::I32,
                                    )),
                                },
                    ...
                    let init_local = self.alloc_local_typed(
                        "__arr_init".into(),
                        if is_lambda_init {
                            ValType::I32
                        } else {
                    ...
                    let store_value = if is_lambda_init {
                        ...
                        CHIRExpr::new(
                            CHIRExprKind::CallIndirect {
                                type_idx: 0,
                                args: vec![i_as_i64],
                                callee: Box::new(CHIRExpr::new(
                                    CHIRExprKind::Local(init_local),
                                    crate::ast::Type::Int32,
                                    ValType::I32,
                                )),
                            },
                    ...
                    None => {
                            ...
                            || self.class_field_offsets.contains_key(name.as_str())
                        {
                            ...
                            let field_names: Vec<String> = self
                                ...
                                .unwrap_or_default();
                            let args_chir: Vec<(String, CHIRExpr)> = args
                                .iter()
                                .enumerate()
                                .map(|(i, a)| {
                                    let arg = self.lower_expr(a).unwrap_or_else(|_| {
                                        CHIRExpr::new(
                                            CHIRExprKind::Nop,
                                            crate::ast::Type::Int32,
                                            ValType::I32,
                ...
                let param_tys: Vec<ValType> = self
                    .type_ctx
                    .functions
                ...
                let args_chir: Result<Vec<CHIRExpr>, String> = args
                    .iter()
                    .enumerate()
                    .map(|(i, a)| {
                        let mut arg_chir = self.lower_expr(a)?;
            ...
            Expr::Match {
                ...
                let (match_ty, match_wasm_ty) = arms_chir
                    .first()
                    .and_then(|arm| {
                        arm.body
                            .result
                            .as_ref()
                            .map(|r| (r.ty.clone(), r.wasm_ty))
                            .or_else(|| {
                                arm.body.stmts.last().and_then(|s| {
                                    if let crate::chir::CHIRStmt::Expr(e) = s {
                                        Some((e.ty.clone(), e.wasm_ty))
                                    } else {
                                        None
                                    }
                                })
            ...
            Expr::TryBlock {
                ...
                for (name, init_expr) in resources {
                    ...
                    self.local_ast_types
                        .insert(name.clone(), value_chir.ty.clone());
                ...
                if has_catch {
                    ...
                    for stmt in body {
                        if let Ok(s) = self.lower_stmt(stmt) {
                            let flag_get = CHIRExpr::new(
                                CHIRExprKind::Local(err_flag),
                                crate::ast::Type::Int32,
                                ValType::I32,
                            );
                            let not_flag = CHIRExpr::new(
                                CHIRExprKind::Unary {
                                    op: crate::ast::UnaryOp::Not,
                                    expr: Box::new(flag_get),
                                },
                                crate::ast::Type::Bool,
                                ValType::I32,
                            );
                            let body_block = crate::chir::CHIRBlock {
                                stmts: vec![s],
                                result: None,
                            };
                            stmts.push(crate::chir::CHIRStmt::Expr(CHIRExpr::new(
                                CHIRExprKind::If {
                                    cond: Box::new(not_flag),
                    ...
                    if !catch_body.is_empty() {
                        ...
                        let catch_block = crate::chir::CHIRBlock {
                            stmts: catch_stmts,
                            result: None,
                        };
                        let flag_get = CHIRExpr::new(
            ...
            Expr::Some(inner) => {
                ...
                let value_size: i64 = if value_wasm == ValType::I64 || value_wasm == ValType::F64 {
                    8
                } else {
                    4
                };
                ...
                let alloc_call = CHIRExpr::new(
                    CHIRExprKind::Call {
                        func_idx: alloc_idx,
                        args: vec![CHIRExpr::new(
                            CHIRExprKind::Integer(total_size),
                            crate::ast::Type::Int32,
                            ValType::I32,
                        )],
                    },
            ...
            Expr::NullCoalesce { option, default } => {
                ...
                let value_align: u32 = if result_wasm == ValType::I64 || result_wasm == ValType::F64
                {
                    3
                } else {
                    2
                ...
                let then_block = crate::chir::CHIRBlock {
                    stmts: vec![],
                    result: Some(Box::new(value_load)),
                };
                let else_block = crate::chir::CHIRBlock {
                    stmts: vec![],
                    result: Some(Box::new(default_chir)),
                };
                let stmts = vec![crate::chir::CHIRStmt::Let {
                    local_idx: tmp,
                    value: opt_chir,
                }];
            ...
            Expr::Throw(inner) => {
                if let (Some(&err_flag), Some(&err_val)) =
                    (self.err_flag_stack.last(), self.err_val_stack.last())
                {
                    let inner_chir = self.lower_expr(inner)?;
            ...
            Expr::Range { start, end, .. } => {
                ...
                let ptr_local =
                    self.alloc_local_typed(format!("__tmp_{}", self.next_local), ValType::I32);
                let start_local =
                    self.alloc_local_typed(format!("__tmp_{}", self.next_local), ValType::I64);
                let end_local =
                    self.alloc_local_typed(format!("__tmp_{}", self.next_local), ValType::I64);
                ...
                stmts.push(crate::chir::CHIRStmt::Expr(CHIRExpr::new(
                    CHIRExprKind::Store {
                        ...
                        value: Box::new(CHIRExpr::new(
                            CHIRExprKind::Local(start_local),
                            crate::ast::Type::Int64,
                            ValType::I64,
                        )),
                        offset: 0,
                ...
                return Ok(CHIRExpr::new(
                    CHIRExprKind::Block(crate::chir::CHIRBlock {
                        ...
                        result: Some(Box::new(CHIRExpr::new(
                            CHIRExprKind::Local(ptr_local),
                            crate::ast::Type::Struct("Range".to_string(), vec![]),
                            ValType::I32,
                        ))),
                    }),
                    crate::ast::Type::Struct("Range".to_string(), vec![]),
            ...
            Expr::Interpolate(parts) => {
                ...
                for part in parts {
                    let part_expr = match part {
                        crate::ast::InterpolatePart::Literal(s) => CHIRExpr::new(
                            CHIRExprKind::String(s.clone()),
                            Type::String,
                            ValType::I32,
                        ),
                ...
                return Ok(result.unwrap_or_else(|| {
                    CHIRExpr::new(
                        CHIRExprKind::String(String::new()),
                        Type::String,
                        ValType::I32,
                    )
                }));
    ...
    fn try_lower_builtin_method(
        ...
        match obj_ty {
            ...
            Type::Float64 | Type::Float32 => match method {
                "format" if args.len() == 1 => {
            ...
            Type::Array(_) | Type::Struct(_, _) => {
                match method {
                    ...
                    "clone" if args.is_empty() => {
                        ...
                        let src_get = || {
                            CHIRExpr::new(CHIRExprKind::Local(src_local), Type::Int32, ValType::I32)
                        };
                        let dst_get = || {
                            CHIRExpr::new(CHIRExprKind::Local(dst_local), Type::Int32, ValType::I32)
                        };
                        let len_get = || {
                            CHIRExpr::new(CHIRExprKind::Local(len_local), Type::Int32, ValType::I32)
                        };
                        let len_load = CHIRExpr::new(
                            CHIRExprKind::Load {
                                ptr: Box::new(src_get()),
                        ...
                        let i_get = || {
                            CHIRExpr::new(CHIRExprKind::Local(i_local), Type::Int32, ValType::I32)
                        };
                        let cond = CHIRExpr::new(
                            CHIRExprKind::Binary {
                                op: crate::ast::BinOp::Lt,
                                left: Box::new(i_get()),
                                right: Box::new(len_get()),
                            },
                            Type::Bool,
                            ValType::I32,
                        );
                        ...
                        let loop_body = crate::chir::CHIRBlock {
                            stmts: vec![
                                crate::chir::CHIRStmt::Expr(CHIRExpr::new(
                                    CHIRExprKind::Store {
                                        ptr: Box::new(dst_elem_ptr),
                                        value: Box::new(src_elem),
                                        offset: 0,
                                        align: 3,
                                    },
                                    Type::Unit,
                        ...
                        all_stmts.push(crate::chir::CHIRStmt::Let {
                            local_idx: i_local,
                            value: CHIRExpr::new(
                                CHIRExprKind::Integer(0),
                                Type::Int32,
                        ...
                        all_stmts.push(crate::chir::CHIRStmt::While {
                            cond,
                            body: loop_body,
                        });
                        return Ok(Some(CHIRExpr::new(
                    ...
                    "slice" if args.len() == 2 => {
                        ...
                        let start_local =
                            self.alloc_local_typed("__slice_start".into(), ValType::I32);
                        let end_local = self.alloc_local_typed("__slice_end".into(), ValType::I32);
                        let new_len_local =
                            self.alloc_local_typed("__slice_len".into(), ValType::I32);
                        ...
                        let end_get = || {
                            CHIRExpr::new(CHIRExprKind::Local(end_local), Type::Int32, ValType::I32)
                        };
                        let new_len_get = || {
                            CHIRExpr::new(
                                CHIRExprKind::Local(new_len_local),
                                Type::Int32,
                                ValType::I32,
                            )
                        };
                        ...
                        let new_len_val = CHIRExpr::new(
                            CHIRExprKind::Binary {
                                op: crate::ast::BinOp::Sub,
                                left: Box::new(end_get()),
                                right: Box::new(start_get()),
                            },
                            Type::Int32,
                            ValType::I32,
                        );
                                ...
                                match method {
                                    ...
                                    "prepend" if args.len() == 1 => {
                                        ...
                                        if let (
                                            ...
                                            let val_local = self.alloc_local_typed(
                                                format!("__tmp_{}", self.next_local),
                                                ValType::I64,
                                            );
                                            let i_local = self.alloc_local_typed(
                                            ...
                                            let loop_body = crate::chir::CHIRBlock {
                                                stmts: vec![
                                                    ...
                                                    crate::chir::CHIRStmt::Assign {
                                                        target: crate::chir::CHIRLValue::Local(
                                                            i_local,
                                                        ),
                                                        value: CHIRExpr::new(
                                                            CHIRExprKind::Binary {
                            ...
                            if sname == "AtomicInt64" {
                                ...
                                match method {
                                    ...
                                    "fetchAdd" if args.len() == 1 => {
                                        ...
                                        let old_local = self.alloc_local_typed(
                                            format!("__tmp_{}", self.next_local),
                                            ValType::I64,
                                        );
                                        let ptr_local = self.alloc_local_typed(
                                    ...
                                    "compareAndSwap" if args.len() == 2 => {
                                        ...
                                        let expected =
                                            self.insert_cast_if_needed(expected, ValType::I64);
                                        let new_val = self.lower_expr(&args[1])?;
                                        let new_val =
                                            self.insert_cast_if_needed(new_val, ValType::I64);
                                        let cur_local = self.alloc_local_typed(
                                            format!("__tmp_{}", self.next_local),
                                            ValType::I64,
                                        );
                                        let ptr_local = self.alloc_local_typed(
                                        ...
                                        let exp_local = self.alloc_local_typed(
                                            format!("__tmp_{}", self.next_local),
                                            ValType::I64,
                                        );
                                        let new_local = self.alloc_local_typed(
                                            format!("__tmp_{}", self.next_local),
                                            ValType::I64,
                                        );
                                        let stmts = vec![
                                        ...
                                        let if_body = crate::chir::CHIRBlock {
                                            stmts: vec![crate::chir::CHIRStmt::Expr(
                                                CHIRExpr::new(
                                                    CHIRExprKind::Store {
                                                        ptr: Box::new(CHIRExpr::new(
                                                            CHIRExprKind::Local(ptr_local),
                                        ...
                                        let cond = CHIRExpr::new(
                                            CHIRExprKind::Binary {
                                                op: crate::ast::BinOp::Eq,
                                                left: Box::new(CHIRExpr::new(
                                                    CHIRExprKind::Local(cur_local),
                                                    Type::Int64,
                                                    ValType::I64,
                                                )),
                                                right: Box::new(CHIRExpr::new(
                                                    CHIRExprKind::Local(exp_local),
                                                    Type::Int64,
                                                    ValType::I64,
                                                )),
                                            },
                        ...
                        crate::ast::Pattern::Struct {
                            ...
                            for (inner_field_name, inner_sub_pat) in inner_fields {
                                let inner_offset = self
                                    .get_field_offset(&inner_ty, inner_field_name)
                                    .unwrap_or(0);
                                let inner_field_ty = self
                                    .type_ctx
                                    .infer_field_type(&inner_ty, inner_field_name)
                                    .unwrap_or(crate::ast::Type::Int64);
                                ...
                                match inner_sub_pat {
                                    crate::ast::Pattern::Literal(lit) => {
                                        ...
                                        chir_fields.push(
                                            crate::chir::StructPatternField::NestedLiteral {
                                                outer_offset: offset,
                                                inner_offset,
                                                value: val,
                                                wasm_ty: inner_wasm,
                                    ...
                                    crate::ast::Pattern::Binding(bind_name) => {
                                        let local_idx =
                                            self.alloc_local_typed(bind_name.clone(), inner_wasm);
                                        self.local_ast_types
                                            .insert(bind_name.clone(), inner_field_ty.clone());
                                        chir_fields.push(
                                            crate::chir::StructPatternField::NestedBinding {
                                                outer_offset: offset,
                                                inner_offset,
                                                local_idx,
                                                wasm_ty: inner_wasm,
...
mod tests {
    use super::*;
    use crate::ast::{BinOp, Program, Stmt, Type};
    ...
    fn test_lower_array_mixed_elems() {
        ...
        let expr = Expr::Array(vec![Expr::Integer(1), Expr::Integer(2), Expr::Float(3.0)]);
        let chir = ctx.lower_expr(&expr).unwrap();
        assert!(matches!(chir.kind, CHIRExprKind::ArrayLiteral { .. }));
    }
