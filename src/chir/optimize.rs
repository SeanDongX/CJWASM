...
pub fn optimize_chir(program: &mut CHIRProgram) {
    ...
    eprintln!(
        "[optimize] inline: calls {} -> {} (eliminated {})",
        before_inline,
        after_inline,
        before_inline - after_inline
    );
    eprintln!(
        "[optimize] locals: lets {} -> {} (eliminated {})",
        before_locals,
        after_locals,
...
fn count_calls(program: &CHIRProgram) -> usize {
    program
        .functions
        .iter()
        .map(|f| count_calls_in_block(&f.body))
        .sum()
}
...
fn count_calls_in_expr(expr: &CHIRExpr) -> usize {
    match &expr.kind {
        ...
        CHIRExprKind::Binary { left, right, .. } => {
            count_calls_in_expr(left) + count_calls_in_expr(right)
        }
...
fn count_locals(program: &CHIRProgram) -> usize {
    program
        .functions
        .iter()
        .map(|f| count_lets_in_block(&f.body))
        .sum()
}
    ...
    match &expr.kind {
        ...
        CHIRExprKind::If {
            ...
            has_side_effects(cond)
                ...
                || else_block
                    .as_ref()
                    .map_or(false, |b| has_side_effects_block(b))
        }
        CHIRExprKind::Block(b) => has_side_effects_block(b),
        CHIRExprKind::Match { subject, arms } => {
            has_side_effects(subject) || arms.iter().any(|a| has_side_effects_block(&a.body))
        }
        _ => false,
    }
}
    ...
    match &expr.kind {
        ...
        CHIRExprKind::If {
            ...
            has_call_to(cond, target_idx)
                ...
                || else_block
                    .as_ref()
                    .map_or(false, |b| has_call_to_block(b, target_idx))
        }
...
fn has_call_to_block(block: &CHIRBlock, target_idx: u32) -> bool {
    block.stmts.iter().any(|s| match s {
        ...
        CHIRStmt::While { cond, body } => {
            has_call_to(cond, target_idx) || has_call_to_block(body, target_idx)
        }
        ...
        .result
        .as_ref()
        .map_or(false, |r| has_call_to(r, target_idx))
}
    ...
    let new_kind = match expr.kind {
        ...
        CHIRExprKind::If {
            ...
            else_block: else_block
                .map(|b| subst_block(b, params, args, caller_next_local, param_count)),
        },
        CHIRExprKind::Block(b) => {
            CHIRExprKind::Block(subst_block(b, params, args, caller_next_local, param_count))
        }
        CHIRExprKind::Match { subject, arms } => CHIRExprKind::Match {
            subject: Box::new(subst_expr(
                *subject,
                params,
                args,
                caller_next_local,
            ...
            arms: arms
                .into_iter()
                .map(|arm| CHIRMatchArm {
                    pattern: subst_pattern(arm.pattern, param_count, caller_next_local),
                    guard: arm
                        .guard
                        .map(|g| subst_expr(g, params, args, caller_next_local, param_count)),
        ...
        CHIRExprKind::Print { arg, newline, fd } => CHIRExprKind::Print {
            arg: arg
                .map(|a| Box::new(subst_expr(*a, params, args, caller_next_local, param_count))),
            newline,
            fd,
        },
        CHIRExprKind::MathUnary { op, arg } => CHIRExprKind::MathUnary {
            op,
            arg: Box::new(subst_expr(
                *arg,
                params,
                args,
                caller_next_local,
...
fn subst_block(
    ...
    CHIRBlock {
        ...
        result: block
            .result
            .map(|r| Box::new(subst_expr(*r, params, args, caller_next_local, param_count))),
    }
}
...
fn subst_stmt(
    ...
    match stmt {
        ...
        CHIRStmt::Expr(e) => {
            CHIRStmt::Expr(subst_expr(e, params, args, caller_next_local, param_count))
        }
        CHIRStmt::Return(Some(e)) => CHIRStmt::Return(Some(subst_expr(
            e,
            params,
            args,
            caller_next_local,
...
fn count_local_reads_in_expr(expr: &CHIRExpr, idx: u32) -> u32 {
    match &expr.kind {
        ...
        CHIRExprKind::Call { args, .. } => {
            args.iter().map(|a| count_local_reads_in_expr(a, idx)).sum()
        }
        CHIRExprKind::MethodCall { receiver, args, .. } => {
            count_local_reads_in_expr(receiver, idx)
                + args
                    .iter()
                    .map(|a| count_local_reads_in_expr(a, idx))
                    .sum::<u32>()
        }
        CHIRExprKind::If {
            cond,
            ...
            count_local_reads_in_expr(cond, idx)
                ...
                + else_block
                    .as_ref()
                    .map_or(0, |b| count_local_reads_in_block(b, idx))
        }
        ...
        CHIRExprKind::TupleNew { elements } | CHIRExprKind::ArrayLiteral { elements } => elements
            .iter()
            .map(|e| count_local_reads_in_expr(e, idx))
            .sum(),
        ...
        CHIRExprKind::Print { arg, .. } => arg
            .as_ref()
            .map_or(0, |a| count_local_reads_in_expr(a, idx)),
        ...
        CHIRExprKind::StructNew { fields, .. } => fields
            .iter()
            .map(|(_, e)| count_local_reads_in_expr(e, idx))
            .sum(),
        CHIRExprKind::Match { subject, arms } => {
            count_local_reads_in_expr(subject, idx)
                + arms
                    ...
                    .map(|a| {
                        a.guard
                            .as_ref()
                            .map_or(0, |g| count_local_reads_in_expr(g, idx))
...
fn count_local_reads_in_block(block: &CHIRBlock, idx: u32) -> u32 {
    let stmt_reads: u32 = block
        .stmts
        .iter()
        .map(|s| count_local_reads_in_stmt(s, idx))
        .sum();
    let result_reads = block
        .result
        .as_ref()
        .map_or(0, |r| count_local_reads_in_expr(r, idx));
...
fn collect_reads(expr: &CHIRExpr, map: &mut HashMap<u32, u32>) {
    match &expr.kind {
        ...
        CHIRExprKind::Binary { left, right, .. } => {
            collect_reads(left, map);
            collect_reads(right, map);
        }
        ...
        CHIRExprKind::If {
            ...
            if let Some(b) = else_block {
                collect_reads_block(b, map);
            }
        }
        CHIRExprKind::Block(b) => collect_reads_block(b, map),
        CHIRExprKind::ArrayGet { array, index } => {
            collect_reads(array, map);
            collect_reads(index, map);
        }
        CHIRExprKind::ArraySet {
            array,
            index,
        ...
        CHIRExprKind::FieldSet { object, value, .. } => {
            collect_reads(object, map);
            collect_reads(value, map);
        }
        CHIRExprKind::Store { ptr, value, .. } => {
            collect_reads(ptr, map);
            collect_reads(value, map);
        }
        ...
        CHIRExprKind::ArrayNew { len, init } => {
            collect_reads(len, map);
            collect_reads(init, map);
        }
...
fn substitute_local_block(block: CHIRBlock, idx: u32, replacement: &CHIRExpr) -> CHIRBlock {
    CHIRBlock {
        ...
        result: block
            .result
            .map(|r| Box::new(substitute_local(*r, idx, replacement))),
    }
}
...
fn eliminate_in_block(
    ...
    while i < block.stmts.len() {
        ...
        if can_elim {
            let (local_idx, value) =
                if let CHIRStmt::Let { local_idx, value } = block.stmts.remove(i) {
                    (local_idx, value)
                } else {
                    unreachable!()
                };
            ...
            if !substituted {
                if let Some(result) = &block.result {
                    ...
                    if reads == 1 {
                        ...
                        block.result =
                            Some(Box::new(substitute_local(result_owned, local_idx, &value)));
                        substituted = true;
                        changed = true;
                    }
                }
...
fn max_written_local_in_stmt(stmt: &CHIRStmt) -> Option<u32> {
    match stmt {
        ...
        CHIRStmt::Assign {
            target: CHIRLValue::Local(idx),
            ..
        } => Some(*idx),
        CHIRStmt::While { body, .. } | CHIRStmt::Loop { body } => max_written_local_in_block(body),
        _ => None,
    }
}
