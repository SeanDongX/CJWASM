...
| 6 | `spawn` 协程 | [Q+A] | ★★★★★ | 极高 | P5 ✅ (单线程桩) |
| 7 | `synchronized` 同步块 | [Q+A] | ★★★★★ | 极高 | P5 ✅ (单线程桩) |
...
| # | 特性 | 来源 | 重要度 | 复杂度 | 阶段 |
|---|------|------|--------|--------|------|
| 30 | `std.collection.*` (HashMap, ArrayList, HashSet, TreeMap) | [Q+A] | ★★★★★ | 极高 | P4 ✅ (HashMap/ArrayList/HashSet/LinkedList) |
| 31 | `std.time.*` (DateTime, TimeZone, Duration, Calendars) | [Q+A] | ★★★★★ | 极高 | P5 ✅ (now() 基于 WASI) |
| 32 | `std.sync.*` (Monitor, AtomicBool, AtomicInt64) | [Q+A] | ★★★★★ | 极高 | P5 ✅ (单线程桩) |
| 33 | `std.log.*` (Logger, LoggerFactory) | [Q+A] | ★★★★☆ | 中 | P5 ✅ (println 包装) |
...
| 53 | `std.sync.*` 扩展 (Mutex, ReentrantMutex, ReentrantReadWriteMutex, Condition) | [A] | ★★★★★ | 极高 | P5 ✅ (Mutex/ReentrantMutex 单线程桩) |
...
### P1：关键 Bug 修复 + 基础语法（预计 3-5 天）✅ 已完成
...
#### 2.5 抽象方法（无 body 的 func 声明） [Q] ✅ 已完成
...
#### 2.7 动态 Array 构造 `Array<T>(size, init)` [ALL] ✅ 已完成
...
#### 2.9 命名参数 `name!: Type = default` [ALL] ✅ 已完成
...
| 阶段 | 版本 | 目标 | 可编译范围 | 状态 |
|------|------|------|-----------|------|
| P1 完成 | v0.9.0 | Bug 修复 + 基础语法 | 避免 enum match / try-catch / prop 等 Bug；支持 `let _ =` | ✅ 已完成 |
| P2 完成 | v0.9.5 | 核心语法补全 | Option 类型、String 方法、static 成员、函数类型、package 声明 | 部分完成 |
| P3 完成 | v1.0.0 | 面向对象支持 | **matrix4cj 核心模块 + quartz4cj 数据模型可编译** | ✅ 已完成 |
| P4 完成 | v1.1.0 | 集合 + 多模块 | **quartz4cj 调度逻辑 + activemq4cj 接口层可编译** | ✅ 部分完成 (集合框架) |
| P5 完成 | v1.2.0 | 并发 + 生态 | **quartz4cj 完整运行** | ✅ 部分完成 (单线程桩) |
...
## 七、与 cjc release/1.0 对比：已知不支持的特性

基于 cjc release/1.0 源码 (Tokens.inc, ASTKind.inc, NodeFormat.fbs, ParserImpl.h, src/Sema/, src/CHIR/, src/CodeGen/) 分析，以下特性 cjwasm **不支持且暂无计划支持**：

### 不支持特性（WASM 沙箱限制）

| 特性 | cjc 状态 | 不支持原因 |
|------|---------|-----------|
| CPointer / CString (C FFI) | 完整 | WASM 沙箱无法链接原生 C 库 |
| unsafe 块 | 完整 | WASM 天然安全，不需要 |
| foreign func + 原生线程/进程 | 完整 | WASM 沙箱限制 |
| 真实 TCP/TLS 网络 | 完整 | WASM 沙箱限制（可通过 WASI socket 扩展） |

### 不支持特性（优先级低 / 复杂度极高）

| 特性 | cjc 状态 | 说明 |
|------|---------|------|
| 宏系统 (macro/quote) | 完整 | 编译期代码生成，需要宏展开引擎 |
| 条件编译 (@When) | 完整 | 需注解系统和平台检测 |
| 完整注解系统 (@Deprecated/@Frozen 等) | 完整 | 仅支持 @import/@export |
| 自定义注解 | 完整 | 需宏系统 |
| 反射 (TypeInfo) | 完整 | 运行时类型信息开销大 |
| 增量编译 | 完整 | 编译器架构限制 |
| BCHIR 解释器 | 完整 | 常量求值引擎 |
| Mock 包 | 完整 | 测试框架特性 |
| redef 修饰符 | 完整 | 使用场景少 |
| @IfAvailable 表达式 | 完整 | 仅鸿蒙平台使用 |
| 自动微分 (Autodiff) | 完整 | 特殊科学计算场景 |
| 溢出策略注解 (@OverflowWrapping 等) | 完整 | 优先级低 |
| JString (Java 兼容字符串) | 完整 | Java 互操作专用 |
| VArray<T,N> 完整实现 | 完整 | 固定长度值类型数组，可用 Array 替代 |
| Quote 表达式 | 完整 | 需宏系统 |
| `<-` 反箭头 (LetPatternDestructor) | 完整 | 使用场景少 |
| ExceptTypePattern | 完整 | 异常类型模式匹配 |
| for-in 守卫 (pattern guard) | 完整 | 优先级低 |

### P6 新增已支持特性

以下 cjc release/1.0 特性在 P6 中已实现：

| 特性 | 状态 | 说明 |
|------|------|------|
| do-while 循环 | ✅ 已完成 | `do { ... } while (cond)` |
| 可选链 (?.) | ✅ 已完成 | `obj?.field` |
| 尾随闭包 (trailing closure) | ✅ 已完成 | `f(args) { x => body }` |
| !in 运算符 | ✅ 已完成 | `x !in collection` |
| const 声明 | ✅ 已完成 | `const var x = ...` |
| 主构造函数 | ✅ 已完成 | `class Foo(var x: T)` |
| inout 参数 | ✅ 已完成 | `func f(inout x: T)` |
| try-with-resources | ✅ 已完成 | `try (let r = ...) { ... }` |

### 覆盖率估算

cjwasm 覆盖仓颉语言约 **75-80%** 的核心语法特性（P6 完成后提升约 5%）。

---

*文档版本: 5.0.0*
*更新日期: 2026-02-16*
*变更: P6 新增 do-while/可选链/尾随闭包/!in/const/主构造函数/inout/try-with-resources*
*P3: operator func, is/as, if let, ??, match type pattern, match where, 元组, Lambda, 方法重载, 嵌套数组, extend, static init*
*P4: HashMap/HashSet/LinkedList 完整方法, ArrayList 补全, Range 属性 (.start/.end/.step)*
*P5: spawn/synchronized 单线程桩, AtomicInt64/AtomicBool (load/store/fetchAdd/compareAndSwap), Mutex/ReentrantMutex (lock/unlock/tryLock), now() 时间戳*
*P6: do-while, ?. 可选链, trailing closure, !in, const, primary constructor, inout, try-with-resources*
