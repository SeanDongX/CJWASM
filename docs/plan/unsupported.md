...
从四个库的编译失败和适配过程中，共识别出 **55 项**未支持特性和 **6 项** Bug：
...
### D. 已发现的 Bug（已实现但行为不正确）

| # | Bug 描述 | 影响 | 严重度 | 阶段 |
|---|----------|------|--------|------|
| B1 | **enum match 变体区分失败**：所有变体匹配第一个 case | 所有使用 enum match 的代码 | ★★★★★ | P1 |
| B2 | **prop getter body 编译不正确**：返回值错误 | 所有使用 prop 的代码 | ★★★★☆ | P1 |
| B3 | **super() 构造调用参数丢失**：父类字段未初始化 | 类继承代码 | ★★★★☆ | P1 |
| B4 | **String `+` 拼接生成空字符串** | 字符串拼接代码 | ★★★★☆ | P1 |
| B5 | **方法链式调用返回类型不追踪**：`a.foo().bar()` 中 `bar()` 找不到 | Builder 模式代码 | ★★★★☆ | P1 |
| B6 | **try-catch codegen 生成无效 WASM**：i64/i32 类型错误 | 异常处理代码 | ★★★★★ | P1 |
...
#### 1.4 Bug 修复：enum match 变体区分 [B1]

**现状**：`match(enumVal) { case A => 1; case B => 2; case C => 3 }` 对所有变体都返回第一个 case 的值。

**验证代码**：
```cangjie
enum Color { | RED | GREEN | BLUE }
func colorCode(c: Color): Int64 {
    match (c) { case RED => 10; case GREEN => 20; case BLUE => 30 }
}
// 实际输出: RED=10 GREEN=10 BLUE=10（全部返回 10）
```

**根因**：codegen 中 enum match 的比较逻辑可能没有正确加载 discriminant 值，或所有变体的 ordinal 都被编码为 0。

**预计工作量**：0.5 天

---

#### 1.5 Bug 修复：prop getter body [B2]

**现状**：`prop area: Int64 { get() { return this._width * this._height } }` 返回值不正确（返回了 `_width` 而非乘积）。

**预计工作量**：0.5 天

---

#### 1.6 Bug 修复：super() 参数传递 [B3]

**现状**：`class Dog <: Animal { init(name: String) { super(name) } }` 中 `super(name)` 未能将参数传递给父类构造函数。

**预计工作量**：0.5 天

---

#### 1.7 Bug 修复：String `+` 拼接 [B4]

**现状**：`"hello" + " " + "world"` 生成空字符串。

**预计工作量**：0.5 天

---

#### 1.8 Bug 修复：方法链式调用返回类型 [B5]

**现状**：`builder.withInterval(30).withRepeat(5).build()` 报错 "方法未找到: 'withRepeat'"，因为 `withInterval()` 返回的 `ScheduleBuilder` 类型未被追踪。

**根因**：方法调用表达式的返回类型推断不足。`infer_ast_type_with_locals()` 对 `Expr::MethodCall` 未递归推断 receiver 类型。

**预计工作量**：1 天

---

#### 1.9 Bug 修复：try-catch codegen [B6]

**现状**：try-catch 块生成的 WASM 有 i64/i32 类型不匹配错误。
