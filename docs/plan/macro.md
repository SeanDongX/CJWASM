...
> 文档版本: 2026-02-16
> 路线: WASM 宏执行（推荐方案）
> 状态: ✅ 已实施（M1-M5 全部完成，M6 大部分完成）

### 实施文件汇总

| 文件 | 变更内容 | 阶段 |
|------|---------|:----:|
| `Cargo.toml` | 添加 serde/serde_json/wasmtime 依赖，macro-system feature gate | M1, M3 |
| `src/ast/mod.rs` | 所有 AST 类型添加 Serialize/Deserialize；新增 MacroDef、Expr::Quote、Expr::MacroCall、Stmt::MacroExpand | M1, M2 |
| `src/parser/mod.rs` | 解析 macro func、quote(...)、@MacroName[...] / @MacroName(...) | M2 |
| `src/codegen/mod.rs` | 新增 MacroExpand/Quote/MacroCall 占位 match arm；更新 Program 构造 | M2, M3 |
| `src/monomorph/mod.rs` | 新增 Stmt::MacroExpand 替换逻辑；更新 Program 构造 | M2 |
| `src/lib.rs` | 声明 pub mod macro_expand | M5 |
| `src/macro_expand/mod.rs` | MacroExpander 主逻辑：提取宏定义、展开宏调用、AST 参数替换 | M5 |
| `src/macro_expand/builtin.rs` | 内建宏 @Log/@Debug/@Stringify/@Todo 的 Rust 原生实现 | M6 |
| `src/macro_expand/runtime.rs` | wasmtime WASM 宏编译与执行运行时（feature-gated） | M3, M5 |
| `src/pipeline.rs` | 编译管线集成宏展开阶段 | M5 |
| `src/main.rs` | CLI 编译命令集成宏展开与错误处理 | M5 |
| `examples/macro_basic.cj` | 宏系统端到端示例（用户宏 + 内建宏 + @Assert/@Expect） | M6 |
...
- [x] 为 `src/ast/mod.rs` 中所有类型添加 `#[derive(Serialize, Deserialize)]`
...
- [x] 添加 `serde` + `serde_json` 依赖到 `Cargo.toml`
- [x] 编写 AST 序列化/反序列化单元测试
- [x] 验证往返一致性：`AST → JSON → AST` 无损
...
- [x] 在 `src/ast/mod.rs` 中添加新的 AST 节点：
  ```rust
  // 宏函数声明
  pub struct MacroDef {
      pub name: String,
...
- [x] 在 `src/codegen/mod.rs` 中实现宏编译流程：
...
- [x] 在 cjwasm 中实现 `std.ast` 的内建映射：
  - 这些类型在宏 WASM 模块中以 JSON 操作实现
  - `Tokens` → JSON 数组
  - `Expr` / `Stmt` → JSON 对象（与 Phase M1 格式一致）
- [x] 编译器识别 `import std.ast.*` 并注入桩函数
- [x] 实现 `quote(...)` 到 JSON 构建代码的编译
- [x] 单元测试：`std.ast` API 可在宏中使用
...
- [x] 添加 `wasmtime` 依赖到 `Cargo.toml`（已通过用户 wasmtime CLI 验证可用）
- [x] 实现宏展开管线 `src/macro_expand/mod.rs`：
  ```rust
...
- [x] 用新宏系统重实现 `@Assert(a, b)`：
  ...
  ```
- [x] 保留编译器内建 `@Assert`/`@Expect` 作为回退（无宏系统时使用）
- [ ] 实现 `@Test` 宏（标记测试函数，收集到测试入口）
- [x] 编写示例文件 `examples/macro_basic.cj`

**验收**：⚠️ 现有测试全部通过（宏版 @Assert 与内建版行为一致）；`@Test` 宏待实现
...
| **M1** | AST 序列化 | 1 周 | 无 | ✅ 完成 |
| **M2** | 宏声明解析 | 1 周 | M1 | ✅ 完成 |
| **M3** | 宏编译到 WASM | 2 周 | M2 | ✅ 完成 |
| **M4** | std.ast 桩库 | 2 周 | M3 | ✅ 完成 |
| **M5** | 宏展开引擎 | 2 周 | M3 + M4 | ✅ 完成 |
| **M6** | 内建宏迁移 + @Test | 1 周 | M5 | ⚠️ 大部分完成（@Test 待实现） |
| **总计** | | **9 周** | | |
