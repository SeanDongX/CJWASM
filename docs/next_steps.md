...
| 14 标准库 | 内置数学(min/max/abs), **print/println(所有基础类型)**, **字符串插值**, **eprint/eprintln**, **类型转换运行时**, **format()**, **ArrayList/HashMap/HashSet/LinkedList/ArrayStack**, **now()/randomInt64()/randomFloat64()**, **getArgs()/getEnv()/exit()**, **sort()**, **String.contains/indexOf/replace/split/toArray/substring** | ✅ 全部完成 |
...
*文档版本: 10.5.0*
*最后更新: 2026-02-14（Phase 7 全部完成：7.4 format 方法 + 7.5 集合类型(ArrayList/HashMap/HashSet/LinkedList/ArrayStack) + 7.6 WASI 扩展(13 个导入+_start) + 7.7 time/random/env(now/randomInt64/randomFloat64/getArgs/getEnv/exit) + 7.8 sort/string(sort/contains/indexOf/replace/split/toArray/substring)；修复 print_i64 缓冲区 off-by-one bug）*
